...
const pwdSchema = z.string().min(6, "A senha deve ter no mínimo 6 caracteres").max(72);
...
const friendlyError = (msg: string) => {
  ...
  if (m.includes("email not confirmed")) return "Confirme seu email para continuar";
...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "signup") {
        ...
        // Save first name for greetings
        const firstName = trimmedName.split(" ")[0];
        try { localStorage.setItem("firstName", firstName); } catch {}
      ...
      const raw = err?.issues?.[0]?.message || err?.message || "Algo deu errado";
  ...
  const loadingLabel =
    mode === "signup" ? "Criando sua conta..." : mode === "forgot" ? "Enviando..." : "Entrando...";
  const submitLabel =
